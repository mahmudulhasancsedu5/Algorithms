...
int tree_node_count(node* rt)
{
    if(rt==NULL)
    {
        return 0;
    }
    int l_node=tree_node_count(rt->left);
    int r_node=tree_node_count(rt->right);

    return l_node+r_node+1;
}

int tree_height(node* rt)
{
    if(rt==NULL) return 0;
    int l_height=tree_height(rt->left);
    int r_height=tree_height(rt->right);

    return 1+max(l_height,r_height);

}
void level_order_traversal(node* rt)
{
    queue<node*>qq;
...
int main()
{
    ...
    cout<<endl<<"Number of nodes in the binary tree = "<<endl;
    cout<<tree_node_count(root)<<endl;

    cout<<endl<<"Binary tree height = ";
